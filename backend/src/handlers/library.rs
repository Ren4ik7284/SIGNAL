...
pub async fn get_library(
    ...
    if let Ok(claims) = extract_claims_from_headers(&headers) {
        ...
        let updated_at: i64 = meta_row.and_then(|r| r.get("updated_at")).unwrap_or(0);

        let track_rows = sqlx::query(
            r#"
            SELECT id, title, artist, album, duration, audio_url, cover_url, genre, format, bitrate, plays, is_favorite, is_live_stream, is_local_upload, playlist_only, added_at
            FROM tracks
            WHERE user_id = ?
            "#,
        )
...
pub async fn save_library(
    ...
    if let Ok(claims) = extract_claims_from_headers(&headers) {
        let user_id = claims.sub;
        let updated_at = data.get("updated_at").and_then(|v| v.as_i64()).unwrap_or_else(|| chrono::Utc::now().timestamp_millis());
